...
os.makedirs(os.path.expanduser('./matrices/gene_drug/'), exist_ok=True)
with open(os.path.expanduser('./config/compound_gene_connectivity.json'), 'r') as file:
...
query = """
MATCH (c:Compound)-[r]->(p:Gene)
WHERE type(r) IN $rel_types
RETURN coalesce(c.name, elementId(c)) AS compound_id, type(r) AS connection_type, coalesce(p.name, elementId(p)) AS gene_id
...
if not df.empty:
    ...
    with open(os.path.expanduser('./matrices/gene_drug/compound_rows.json'), 'w') as f:
        json.dump(row_labels, f)

    with open(os.path.expanduser('./matrices/gene_drug/gene_cols.json'), 'w') as f:
        json.dump(col_labels, f)

    numpy_matrix = pure_matrix.to_numpy(dtype=np.int8)
    np.save(os.path.expanduser('./matrices/gene_drug/dense_adjacency_matrix.npy'), numpy_matrix)

    sparse_matrix = csr_matrix(numpy_matrix)
    save_npz(os.path.expanduser('./matrices/gene_drug/sparse_adjacency_matrix.npz'), sparse_matrix)
    ...
    print(df[df['matrix_value'] == 1][['compound_id', 'gene_id', 'connection_type']].head(10))
    print("\n")
